"""ORCA recipe: nf-core/sarek WES germline variant calling on GRCh38.
Implements EAGER-germline-1 (https://sagebionetworks.jira.com/browse/EAGER-germline-1);
validated version lives in sagebio-ada/orca-recipes PR#1, this is an optimized copy.
...
Orchestrates four steps:
  ...
  3. sarek                : Run nf-core/sarek germline variant calling
...
Usage:
  ...
  python sarek_germline_workflow.py
  python sarek_germline_workflow.py --run-number 2 # with a specific run number


  # Run individual steps (e.g. fetch_samplesheet, synstage, sarek, synindex)
  python sarek_germline_workflow.py fetch_samplesheet
  python sarek_germline_workflow.py synstage
  python sarek_germline_workflow.py sarek
  python sarek_germline_workflow.py synindex

  # Run multiple steps (e.g. synstage + sarek)
  python sarek_germline_workflow.py synstage sarek
  python sarek_germline_workflow.py synstage sarek --run-number 2 # with a specific run number
  ...
  python sarek_germline_workflow.py sarek --run-number 2
  python sarek_germline_workflow.py synindex --run-number 2
"""
import asyncio
import argparse
